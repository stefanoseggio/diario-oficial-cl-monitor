...
- **No credential handling of any kind.** This Actor requires no third-party API key or BYOK secret (see the README's Cost & BYOK Disclosure section) — the source, Chile's Diario Oficial (`diariooficial.interior.gob.cl`), is a public government gazette with no login wall or provider key of any kind. There is no customer secret this Actor could leak.
- **No user-supplied code execution.** Input is a fixed JSON schema (`sections`, `maxItems`, `onlyNew`, `eventTypes`) validated against the Actor's own `.actor/input_schema.json`, with `sections` and `eventTypes` constrained to a fixed enum — there is no arbitrary-code or arbitrary-URL input surface.
...
- **Source-feed integrity** (a compromised or spoofed `diariooficial.interior.gob.cl` endpoint) is outside this Actor's control — it fetches from the source's own official government-published URL over HTTPS and does not implement independent content-signing verification beyond standard TLS.
